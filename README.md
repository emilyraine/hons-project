...
This fork includes two primary extensions for the honours project: 
...
The naheed-fixedMorph branch contains the code implementation of dog agents with fixed morphology and the new environments.
...
`process/plotmaxxer.py` is the project's plotting helper used to create the figures in and `figures/`.

### Expected directory layout (inputs)

The plotting tools expect per-run CSV results under `output/CSVresults/` organised by treatment, environment and dog-count. Example layout:

```
output/CSVresults/
	evolving/
		<environment>/       # e.g. field, ice, ice_mud_high, mud, ...
			<dogs>/            # e.g. 15dogs, 20dogs
				run_<id>/
					results.csv   # per-run CSV (must include a `gen` column)
	fixed/
		<environment>/
			<dogs>/
				run_<id>/
...
Two helper scripts included in `scripts/` produce compact summary CSVs that `plotmaxxer` can consume:

- `scripts/average_csvs.py` — groups runs by environment/dogs, aligns on `gen` and writes:
	- `output/CSVresults/evolving_summary.csv`
	- `output/CSVresults/fixed_summary.csv`

- `scripts/extract_gen100.py` — filters the summary files for `gen == 100` and writes:
	- `output/CSVresults/evolving_gen100.csv`
	- `output/CSVresults/fixed_gen100.csv`

Create summaries before plotting (if not already present):

```bash
python3 scripts/average_csvs.py
python3 scripts/extract_gen100.py   # optional: creates *_gen100.csv files
```

### Dependencies

Install plotting and data libraries (use your `roborobo` environment):
